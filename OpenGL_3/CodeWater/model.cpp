 ...
 */
void Model::unitize() {
    QVector3D maxima = QVector3D(-INFINITY,-INFINITY,-INFINITY);
    QVector3D minima = QVector3D(INFINITY,INFINITY,INFINITY);
    for (int i = 0; i < vertices.size(); i++) {
        QVector3D vertex = vertices[i];
        maxima.setX(std::max(vertex.x(), maxima.x()));
        maxima.setY(std::max(vertex.y(), maxima.y()));
        maxima.setZ(std::max(vertex.z(), maxima.z()));

        minima.setX(std::min(vertex.x(), minima.x()));
        minima.setY(std::min(vertex.y(), minima.y()));
        minima.setZ(std::min(vertex.z(), minima.z()));

    }

    QVector3D mean = (maxima + minima) / 2;

    float maximum = std::max(std::max(maxima.x(), maxima.y()), maxima.z());
    float minimum = std::min(std::min(minima.x(), minima.y()), minima.z());
    maximum = std::max(std::abs(minimum), maximum);

    for (int v = 0; v < vertices.size(); v++) {
        vertices[v].setX((vertices[v].x()-mean.x()) / maximum);
        vertices[v].setY((vertices[v].y()-mean.y()) / maximum);
        vertices[v].setZ((vertices[v].z()-mean.z()) / maximum);
    }
}
